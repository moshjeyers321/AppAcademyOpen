...
def first_index(array, &prc)
    array.each {|el| return array.index(el) if prc.call(el)}
    nil
end
...
def xnor_select(array, prc_1, prc_2)
    array.select {|el| el if (prc_1.call(el) && prc_2.call(el)) ^ (!prc_1.call(el) && !prc_2.call(el))}
...
def filter_out!(array, &prc)
    array.select! { |el| el if !prc.call(el)}
end
...
def proctition_platinum(array, *procs)
    ...
    (1..procs.length).each {|i| hash[i] = Array.new}
