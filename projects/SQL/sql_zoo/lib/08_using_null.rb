...
def teachers_and_depts
  ...
  # department.
  execute(<<-SQL)
  SELECT teachers.name, COALESCE(depts.name, 'None') as departments
  FROM teachers
  LEFT JOIN depts on depts.id = teachers.dept_id;
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT COUNT(teachers.name) as teacher_count, COUNT(teachers.mobile) as phone_count
  FROM teachers;
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  SELECT depts.name, COUNT(teachers.*)
  FROM depts
