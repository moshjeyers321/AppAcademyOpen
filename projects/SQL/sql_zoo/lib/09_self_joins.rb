...
def haymarket_and_leith
  ...
  JOIN
    routes AS end_routes ON end_routes.company = start_routes.company 
      AND end_routes.num = start_routes.num
  WHERE
...
def craiglockhart_and_tollcross
  ...
  JOIN
    routes AS end_routes ON end_routes.company = start_routes.company
      AND end_routes.num = start_routes.num
  JOIN
    stops AS stops_a ON stops_a.id = start_routes.stop_id 
  JOIN
    stops AS stops_b ON stops_b.id = end_routes.stop_id
  WHERE
    stops_a.name = 'Craiglockhart'
      AND stops_b.name = 'Tollcross';
